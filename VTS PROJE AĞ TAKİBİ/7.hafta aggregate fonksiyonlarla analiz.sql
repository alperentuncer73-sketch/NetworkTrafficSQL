...
-- 1. Her cihaza ait toplam trafik miktarını bulma
-- GROUP BY ile cihaz bazlı gruplama yapýcaz
...
-- 2. kullanıcılara göre trafik analizi yapma
-- Hangi departman veya kişinin ağı yorduğunu bulabilme
...
SELECT 
    ...
    AVG(T.DataTransferredMB) AS Ortalama_Islem_Boyutu_MB -- Ortalama işlem hacmi
FROM 
    TrafficLogs AS T
JOIN 
...
SELECT 
    CAST(T.EventTimestamp AS DATE) AS Gun, -- Saati atıp sadece tarihi aldık
...
-- 4. en yüksek ve en düşük trafik değerlerini bulma

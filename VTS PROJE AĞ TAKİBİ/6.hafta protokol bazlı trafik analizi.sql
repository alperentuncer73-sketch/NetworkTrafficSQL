...
--  Protokol bazında toplam trafik miktarını hesaplama
...
SELECT
    ...
    COUNT(T.LogID) AS Baglanti_Sayisi,        -- Kaç kez kullanılmış?
...
--  En çok kullanılan protokolü bulucaz
...
SELECT
    P.ProtocolName AS Protokol,
    AVG(T.DataTransferredMB) AS Ortalama_Veri_MB, -- Bağlantı başına ortalama boyut
    AVG(T.PacketCount) AS Ortalama_Paket_Sayisi   -- Bağlantı başına ortalama paket
FROM 
    TrafficLogs AS T
JOIN 
    TrafficProtocols AS P ON T.ProtocolID = P.ProtocolID
...
-- Filtreleme örneği TCP kayıtları için bakalım
